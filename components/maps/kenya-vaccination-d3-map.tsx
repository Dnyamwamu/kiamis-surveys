...
export default function KenyaVaccinationD3Map() {
  const topology = useTopology();

  const data = useMemo(() => {
    return DUMMY_COUNTY_DATA.map((dummy) => {
      const totalFarmers = dummy.maleFarmers + dummy.femaleFarmers;
      const dosesAdministered = Math.floor(totalFarmers * 0.84); // Mock 84% vaccine dose registration rate
      const coverageRate = totalFarmers > 0 ? (dosesAdministered / totalFarmers) * 100 : 0;
      return {
        county: dummy.county,
        totalFarmers,
        dosesAdministered,
  ...
  const { maxValue, colorScale } = useMemo(() => {
    const maxVal = Math.max(...data.map((d) => d.dosesAdministered), 1);
    return {
      maxValue: maxVal,
      colorScale: (value: number) => {
        const intensity = Math.min(value / maxVal, 1);
        const hue = 205; // Deep clinical blue
  ...
  const mapData = useMemo(() => {
    return data.reduce((acc, d) => {
      acc[d.county.toUpperCase()] = {
        value: d.dosesAdministered,
        color: colorScale(d.dosesAdministered),
      };
      return acc;
  ...
  const totalDosesAdministered = useMemo(() => {
    return data.reduce((sum, d) => sum + d.dosesAdministered, 0);
  }, [data]);

  const averageCoverageRate = useMemo(() => {
    return data.reduce((sum, d) => sum + d.coverageRate, 0) / data.length;
  }, [data]);
    ...
    return (
      <Card className="border border-gray-200 shadow-sm">
        <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
          <CardTitle className="text-xl font-bold text-gray-900">Vaccination Distribution Map (D3)</CardTitle>
          <CardDescription>
            Geographic distribution of livestock vaccinations administered using the D3 vector map
          </CardDescription>
        </CardHeader>
    ...
    <Card className="overflow-hidden border border-gray-200 shadow-sm">
      <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
        <CardTitle className="text-xl font-bold text-gray-900">
          Livestock Vaccination Map
        </CardTitle>
        <CardDescription>
          Clinical Deep Blue/Teal colors highlighting cattle & livestock immunization coverage across all 47 counties.
        </CardDescription>
      </CardHeader>
                ...
                return (
                  <div className="space-y-1 p-1">
                    ...
                    <div className="text-xs text-gray-700">
                      Doses Administered: <span className="font-semibold text-blue-700">{countyStats.dosesAdministered.toLocaleString()}</span>
                    </div>
                    <div className="text-[10px] text-gray-500">
                      Coverage Rate: {countyStats.coverageRate.toFixed(1)}% | Registered Farmers: {countyStats.totalFarmers.toLocaleString()}
                    </div>
                  </div>
                );
              }}
          ...
          <div className="w-full lg:w-[420px] flex flex-col gap-6">
                ...
                <table className="w-full text-xs text-left">
                  <thead className="sticky top-0 bg-gray-50 z-10 text-gray-500 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="py-2 px-3 font-semibold text-right">Doses Admin.</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {data
                      .sort((a, b) => b.dosesAdministered - a.dosesAdministered)
                      .map((county, index) => (
                        <tr key={county.county} className="hover:bg-blue-50/30 transition-colors">
            ...
            <div className="border border-gray-100 p-4 rounded-xl bg-gray-50/40 space-y-2">
              ...
              <div className="space-y-2 text-xs font-medium">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Total Doses Administered</span>
                  <span className="text-blue-700 font-bold text-sm">
                    {totalDosesAdministered.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Average Coverage Rate</span>
                  <span className="text-gray-900 font-bold">
                    {averageCoverageRate.toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
