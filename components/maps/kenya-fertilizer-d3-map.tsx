...
export default function KenyaFertilizerD3Map() {
  const topology = useTopology();

  const data = useMemo(() => {
    return DUMMY_COUNTY_DATA.map((dummy) => {
      const totalFarmers = dummy.maleFarmers + dummy.femaleFarmers;
      const redeemedVouchers = Math.floor(totalFarmers * 0.72); // Mock 72% voucher redemption rate
  ...
  const { maxValue, colorScale } = useMemo(() => {
    ...
    return {
      ...
      colorScale: (value: number) => {
        ...
        const hue = 145; // Forest/Emerald green
        const saturation = 45 + intensity * 45;
    ...
    return (
      <Card className="border border-gray-200 shadow-sm">
        <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
          <CardTitle className="text-xl font-bold text-gray-900">Fertilizer Distribution Map (D3)</CardTitle>
          <CardDescription>
            Geographic distribution of fertilizer voucher redemptions using the D3 vector map
          </CardDescription>
        </CardHeader>
    ...
    <Card className="overflow-hidden border border-gray-200 shadow-sm">
      <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
        <CardTitle className="text-xl font-bold text-gray-900">
          Fertilizer Voucher Redemption Map
        </CardTitle>
        <CardDescription>
          Micro-animation zoombale vectors illustrating localized fertilizer subsidy redemptions across all 47 counties.
        </CardDescription>
      </CardHeader>
                ...
                return (
                  <div className="space-y-1 p-1">
                    ...
                    <div className="text-xs text-gray-700">
                      Vouchers Redeemed: <span className="font-semibold text-emerald-700">{countyStats.redeemedVouchers.toLocaleString()}</span>
                    </div>
          ...
          <div className="w-full lg:w-[420px] flex flex-col gap-6">
            ...
            <div>
              <h4 className="text-sm font-semibold mb-2 text-gray-900">
                Counties by Fertilizer Redemption
              </h4>
                ...
                <table className="w-full text-xs text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {data
                      ...
                      .map((county, index) => (
                        <tr key={county.county} className="hover:bg-emerald-50/30 transition-colors">
