...
export interface KenyaFarmersD3MapProps {
...
export default function KenyaFarmersD3Map({
  ...
  const data = useMemo(() => {
    if (surveyData) {
      return surveyData.map((item) => {
        return {
          county: item.county,
          totalFarmers: item.visited,
          maleFarmers: Math.round(item.visited * 0.52),
          femaleFarmers: item.visited - Math.round(item.visited * 0.52),
          target: item.target,
  ...
  const totalFarmersPopulation = useMemo(() => {
    return data.reduce((sum, d) => sum + d.totalFarmers, 0);
  }, [data]);

  if (topology == null) {
    if (showCardWrapper === false) {
      return (
        <div className="w-full h-full flex items-center justify-center py-20 bg-slate-50/50 rounded-xl border border-slate-200">
          <Skeleton className="h-10 w-10 animate-spin text-emerald-600 rounded-full border-4 border-emerald-600 border-t-transparent" />
        </div>
      );
    }
    return (
      <Card className="border border-gray-200 shadow-sm">
        <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
          <CardTitle className="text-xl font-bold text-gray-900">Farmer Registration Map (D3)</CardTitle>
          <CardDescription>
            Geographic distribution of farmer registrations using the custom D3 map module
          </CardDescription>
        </CardHeader>
            ...
            return (
              <div className="space-y-1 p-1">
                ...
                {surveyData ? (
                  <>
                    <div className="text-xs text-gray-700">
                      Target: <span className="font-semibold text-slate-700">{target.toLocaleString()}</span>
                    </div>
                    <div className="text-xs text-gray-700">
                      Completion: <span className="font-semibold text-emerald-700">{percent}%</span>
                    </div>
                  </>
                ) : (
                  <div className="text-[10px] text-gray-400">
                    Male: {countyStats.maleFarmers.toLocaleString()} | Female: {countyStats.femaleFarmers.toLocaleString()}
                  </div>
                )}
              </div>
            );
    ...
    <Card className="overflow-hidden border border-gray-200 shadow-sm">
      <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
        <CardTitle className="text-xl font-bold text-gray-900">
          Farmer Registration Map
        </CardTitle>
        <CardDescription>
          High-performance D3 vector-rendered boundaries with micro-animation zooms and case-insensitive region keys.
        </CardDescription>
      </CardHeader>
              ...
              tooltipContents={(regionKey) => {
                ...
                return (
                  <div className="space-y-1 p-1">
                    <div className="font-bold text-gray-900">{countyStats.county}</div>
                    <div className="text-xs text-gray-700">
                      Total Registered: <span className="font-semibold text-green-700">{countyStats.totalFarmers.toLocaleString()}</span>
                    </div>
                    <div className="text-[10px] text-gray-400">
                      Male: {countyStats.maleFarmers.toLocaleString()} | Female: {countyStats.femaleFarmers.toLocaleString()}
                    </div>
                ...
                <table className="w-full text-xs text-left">
                  <thead className="sticky top-0 bg-gray-50 z-10 text-gray-500 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="py-2 px-3 font-semibold text-right">Male</th>
                      <th className="py-2 px-3 font-semibold text-right">Female</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {data
                      ...
                      .map((county, index) => (
                        <tr key={county.county} className="hover:bg-green-50/30 transition-colors">
                          ...
                          <td className="py-2 px-3 text-right text-gray-500">
                            {county.maleFarmers.toLocaleString()}
                          </td>
                          <td className="py-2 px-3 text-right text-gray-500">
                            {county.femaleFarmers.toLocaleString()}
                          </td>
                        </tr>
                      ))}
                  </tbody>
            ...
            <div className="border border-gray-100 p-4 rounded-xl bg-gray-50/40 space-y-2">
              ...
              <div className="space-y-2 text-xs font-medium">
                <div className="flex justify-between">
                  <span className="text-gray-500">Total Counties</span>
                  <span className="text-gray-900 font-bold">{data.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Total Registered Farmers</span>
