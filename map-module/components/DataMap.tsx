...
export const DataMap = <T extends MapDatum>(props: Props<T>) => {
  ...
  const zoom = useMemo(() => {
    return d3.zoom<SVGSVGElement, unknown>().scaleExtent([0.5, 4]);
  }, []);

  const [hoveredRegion, setHoveredRegion] = useState<string | null>(null);

  const path = useMemo(() => {
    if (!svgRef.current || width == null || height == null) {
      return null;
    }
  ...
  }, [topology, height, width, projectionType]);
  ...
  useLayoutEffect(() => {
    ...
    areasGroup
      ...
      .on('click', (event, d) => {
        const link = linkedRegions?.[keyForRegion(d.properties)];
        if (link != null) {
          router.push(link);
        }
      });
