  ...
  const { mapData, regionNamesByKey, colorScale, minValue, maxValue } =
    useMemo(() => {
      ...
      regions.forEach((region) => {
        const key = region.alpha3Code ? region.alpha3Code.toLowerCase() : region.name;
      ...
      const regionNamesByKey = regions.reduce(
        (result, region) => {
          result[
            region.alpha3Code != null
              ? region.alpha3Code.toLowerCase()
              : region.name
          ] = region.name;
          return result;
        },
      ...
      const mapData = regions.reduce(
        (result, region) => {
          ...
          if (adminLevel === 'national') {
            ...
            // Chinese Aksai Chin exceptional styling
            if (region.alpha3Code === 'CHN') {
              result['xac'] = {
                ...datum,
                color: scale.colorScale(datum.value),
              };
          ...
          {topology != null && (
            <DataMap
              ...
              tooltipContents={(regionKey, datum) => {
                const regionName = regionNamesByKey[regionKey];
